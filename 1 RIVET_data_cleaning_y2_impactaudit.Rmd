...
```{r}
# code each case by exclusion criteria:
df_rivet = df_rivet |>
  mutate(
    ...
    paused_out = if_else(degree_type == "BA"     & enrollment_status %in% c("3 Paused A", "3 Paused B"), 1, 0),
    withdrawn_out = if_else(degree_type == "BA"  & enrollment_status %in% c("4 Withdrawn"), 1, 0)
    )
...
pause_out_df = df_rivet |>
  filter(!id %in% c(e_out_df$id, aa_out_df$id, c_out_df$id, active_out_df$id),
         paused_out == 1)
...
```{r}
# this asks if the exclusions plus the individuals in the impact metric plus the individuals removed bc they were in last years data add up to the number of cases in the raw data set
